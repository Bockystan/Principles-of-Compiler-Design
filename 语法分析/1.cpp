...
/*
一个非终结符，若它存在某个候选首符集包含ε，FIRST(A)∩FOLLOW(A)=Φ，则满足LL（1）文法
LL(1)文法是非回溯且自顶向下的文法
预测分析表用一个M[A,a]形式的矩阵表示，存放着一条关于A的产生式
...
预测分析表：
对每个非终结符a属于FIRST（α），把A->α放到M[A,a]中。（A在栈顶，匹配）
对ε属于FIRST（α），对任何属于FOLLOW（A）把A->α放到M[A,a]中（匹配成功）
无定义：error
...
若为非终结符 查表
若M[X,a]中存放着关于X的一个产生式，则X出栈，把产生式倒序入栈，若为空 不推进栈
*/
using namespace std;
//构建first
class WF
